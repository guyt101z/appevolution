...
if ($action == 'Export') {
	...
	export_table_csv(TABLE_COUPONS, $export_file_name);
	exit();
}

$titlex = "Coupons export(.csv)";
...
<form name="" encType="multipart/form-data" method="post" autocomplete="off" class="edit_form" id="EditForm" target="export_result">
	CVS file name: <input type="text" name="export_file_name" id="export_file_name" value="coupons_list_<?= date('YmdHis')?>.csv" style="width: 250px;" class="validate[required]"/>
