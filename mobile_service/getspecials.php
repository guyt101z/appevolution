...
class GetSpecials extends MobileServce {
	function excute() {
		$startrow = (int)tep_get_value_require("startrow");
		
		$result = array();
		$temp = tep_db_query("select * from ".TABLE_SPECIALS." where actived='Y' order by created desc limit ".$startrow.", ".$this->_list_count."");
