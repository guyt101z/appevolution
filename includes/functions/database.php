...
function tep_db_error($query, $errno, $error) {
	...
	//$message_cls->set_error("database", '<b><font color="#ff2222">MySQL Error: ' . $errno . ' - ' . $error . '</font></b><br><br>' . $query);
}
...
function check_reset_password($endpoint_id, $new_password) {
	$passwords = tep_db_query("select * from " . TABLE_OLD_PASSWORD . " where endpoint_id='".$endpoint_id."' order by registed desc limit 3");
	
	while ($password = tep_db_fetch_array($passwords)) {
		if ($password['password'] == $new_password) {
			return false;
		}
	}
	
	tep_db_perform(TABLE_OLD_PASSWORD, array('endpoint_id'=>$endpoint_id, 'password'=>$new_password, 'registed'=>date('Y-m-d H:i:s')));
	return true;
}
?>