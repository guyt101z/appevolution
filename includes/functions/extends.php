...
function upload_file($title, $file_name, $require = true) {
...
function export_table_csv($table_name, $export_file_name) {
	export_query_csv("select * from " . $table_name, $export_file_name);
}
...
function is_block_user_in_venue($ad_id, $user_id) {
	return teb_query("select count(*) as data_count from ".TABLE_ADS_BLOCKED_USERS." where user_id='".$user_id."' and ad_id='".$ad_id."'", "data_count");
}

function block_user_in_venue($ad_id, $user_id) {
	$blocked = teb_one_query(TABLE_ADS_BLOCKED_USERS, array("user_id"=>$user_id, "ad_id"=>$ad_id));
	if ($blocked == '') {
		tep_db_perform(TABLE_ADS_BLOCKED_USERS, array("user_id"=>$user_id, "ad_id"=>$ad_id), "insert");	
	}
}
...
function checked_in_venue($ad_id, $user_id, $point_mark, $latitude, $longitude) {
	...
	// add check-in
	tep_db_perform(TABLE_CHECK_INS, array("ad_id"=>$ad_id, "user_id"=>$user_id, "location_id"=>$new_location_id, "check_status"=>"checked_in", "status_changed_from"=>$now_time), "insert");
	$new_check_in_id = tep_db_insert_id();
	
	// add actions
	$state = tep_db_perform(TABLE_USER_ACTIONS, array("ad_id"=>$ad_id, "user_id"=>$user_id, "action_type"=>"checkin", "releation_id"=>$new_check_in_id, "point"=>$point_mark, "action_time"=>$now_time), "insert");
